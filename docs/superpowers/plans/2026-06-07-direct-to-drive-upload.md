...
- **Task 15 — DONE (commit `3186010`)** — token-based upload progress bar (ARIA `progressbar`, single dynamic width style) in FormWizard + overall fraction from both forms; `role="alert"`+`aria-live` on the size error. tsc 0; dev server 200 on both apply pages.
- **Task 16 — DONE (commit `49fa8b2`)** — middleware matcher -> `/api/upload/:path*`, `isUpload` via startsWith, uploadLimiter raised 20 -> 300/60s (multi-file bursts); removed dead old `/api/upload/route.ts`. CSP step skipped (Approach B, same-origin). tsc 0.
- **Task 17 — DEFERRED** — stale-quarantine sweep (pure housekeeping; abandoned uploads are harmless orphans). Easy follow-up; not needed for correctness/testing.
- **VERIFIED 2026-06-07:** full unit suite **13/13 passing**; production `next build` **succeeds** (compile + TypeScript + static gen; new endpoints present, old route gone).
- **Task 18 — PENDING KEVIN (touches shared resources):** (1) run the Neon migration `scripts/2026-06-07-create-application-uploads.ts` on the SHARED dev/prod DB (additive: 2 new tables) — needs go-ahead; (2) the definitive >4.5MB test requires a PREVIEW DEPLOY (Vercel's 4.5MB limit only exists in the deployed env, not local dev).
- **MIGRATION RUN (2026-06-07):** `application_uploads` + `application_invoice_locks` created on the shared Neon DB (additive; confirmed). Kevin authorized.
- **PREVIEW DEPLOYED (2026-06-07):** branch pushed to `origin/feat/direct-drive-upload`; Vercel preview `itsrellestate-p274cpth0-kevsosmooths-projects.vercel.app` built GREEN with branch-scoped preview env = prod values EXCEPT dev Google folder IDs + TEST Stripe keys (so test uploads -> DEV Drive folder, Stripe test mode, no real charge). 17 env vars set for the preview branch (16 via `--value`, multiline `GOOGLE_PRIVATE_KEY` via stdin). The env-less first build errored (stripe.ts throws at import) — proves env now applies.
- **PENDING: live >4.5MB test.** Preview is behind Vercel login-protection (401 to curl) — Kevin (account owner) opens it logged in. Build verified; the actual big-file upload is Kevin's to confirm. Branch NOT merged to main.
- Known: Next 16 warns `middleware` is deprecated in favor of `proxy` (pre-existing; works; separate migration).
