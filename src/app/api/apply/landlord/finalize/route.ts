...
import { generateAndUploadLandlordPdf } from "@/lib/pdf-landlord";
import {
  getFolderProperties,
  patchFolderProperties,
  readApplicationJSON,
} from "@/lib/google";
import { landlordSchema } from "@/lib/validation";
...
export async function POST(request: Request) {
  try {
    ...
    const parsed = landlordSchema.safeParse(json);
    if (!parsed.success) {
    ...
    after(async () => {
      try {
        await generateAndUploadLandlordPdf({
          folderId,
          uploadsFolderId,
          data: { ...parsed.data, submittedAt } as Parameters<typeof generateAndUploadLandlordPdf>[0]["data"],
        });
        await patchFolderProperties(folderId, { pdfGenerated: "1" });
      } catch (err) {
        console.error("[pdf] landlord finalize failed:", err);
      }
    });
    ...
    console.error("Landlord finalize error:", message);
